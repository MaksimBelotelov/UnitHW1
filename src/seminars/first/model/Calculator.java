...
public class Calculator {
    public static int calculation(int firstOperand, int secondOperand, char operator) {
        ...
        switch (operator) {
            case '+':
                ...
                if(result < firstOperand && result < secondOperand)
    ...
    public static double calculatingDiscount(double purchaseAmount, int discountAmount)
        ...
        if(discountAmount < 0)
            ...
            throw new ArithmeticException("Скидка не может быть больше 99 %");
        else if(purchaseAmount <= 0)
            throw new ArithmeticException("Сумма покупки не может быть меньше или равной нулю");
        else
            return purchaseAmount * ((100 - discountAmount) * 0.01);
    }
}